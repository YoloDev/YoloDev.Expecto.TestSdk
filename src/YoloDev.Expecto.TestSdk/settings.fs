...
module internal SettingsParser =
  ...
  let string (_: ParserState, x: XElement) =
    if String.IsNullOrWhiteSpace x.Value then None else Some x.Value
  ...
  let readExpectoConfig (logger: YoloDev.Expecto.TestSdk.Logging.Logger) expectoConfig (confNode: Xml.Linq.XElement) =
    let parser =
      SettingsParser.build logger
        [ "sequenced",
          SettingsParser.exclusive "parallel" *> SettingsParser.bool >?> function
